    ...
    RSpec.describe Ledger, :aggregate_failures, :db do
        ...
        describe "#expenses" do
            it 'returns all expenses for the provided date' do
                result_1 = ledger.record(expense.merge('date' => '2020-06-10'))
                result_2 = ledger.record(expense.merge('date' => '2020-06-10'))
                result_3 = ledger.record(expense.merge('date' => '2020-06-11'))

                expect(ledger.expenses_on('2020-06-10')).to contain_exactly(
            ...
            it 'returns a blank array when there are no matching expenses' do
                expect(ledger.expenses_on('2020-06-10')).to eq([])
            end
        end
    end
end